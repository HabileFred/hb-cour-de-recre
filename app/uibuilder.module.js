...
export const Uib = class Uib {
  ...
  set logLevel(level) {
    ...
    console.log(
      '%c❗ info%c [logLevel]',
      `${LOG_STYLES.level} ${LOG_STYLES.info.css}`,
      `${LOG_STYLES.head} ${LOG_STYLES.info.txtCss}`,
      `Set to ${level} (${LOG_STYLES.names[level]})`,
  ...
  start(options) {
    ...
    // Do we need to load styles?
      ...
      } else if (options && options.loadStylesheet === false) log('info', 'Uib:start', 'No styles loaded & options.loadStylesheet === false.')()
            else {
