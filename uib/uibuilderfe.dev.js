...
(function() {
  ...
  var uibuilder = (function uibuilder() { // eslint-disable-line no-extra-parens
    ...
    self.debug = self.isUnminified === true // do not change directly - use .debug() method
    ...
    self.onChange('msg', function onMsgRcvd(msg) {
      ...
      if ( componentRef in vueApp.$refs ) {
        ...
        if ( msg.payload ) {
          self.uiDebug(
            'log',
            '🔢📈 new component instance value received for ref',
            componentRef,
            ':',
            msg.payload,
    ...
    self.uiReturn = {
      ...
      showToast(text, ref = 'globalNotification', options = {}) {
                const msg = {
                    '_uib': {
                        'componentRef': ref,
                        'options': options,
